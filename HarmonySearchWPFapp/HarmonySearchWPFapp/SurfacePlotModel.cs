...
namespace HarmonySearchWPFapp
{
    ...
    class SurfacePlotModel : INotifyPropertyChanged
    {
        ...
        public SurfacePlotModel()
        {
            ...
            Func<double, double, double> sampleFunction = (x, y) => Math.Sin(x)*Math.Cos(y);
            PlotFunction(sampleFunction, -5, 5);
        }

        #region === Public Methods ===


        public void PlotData(double[] xData, double[] yData, double[,] zData)
        {
            int x_len = xData.GetLength(0);
            int y_len = yData.GetLength(0);
            int z_len = zData.GetLength(0);
            Point3D[,] resultArray = new Point3D[x_len, y_len];
            
            for(int i=0; i<x_len; i++)
            {
                for(int j=0; j<y_len; j++)
                {
                    Point3D tmpPoint = new Point3D(xData[i], yData[j], zData[i,j]);
                    resultArray[i, j] = tmpPoint;
                }
            }
